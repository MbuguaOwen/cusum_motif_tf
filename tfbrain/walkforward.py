    ...
    for fi in pbar_folds:
        ...
        if min_test_cands > 0 and len(cands) > 0:
            ...
            if n_test < min_test_cands:
                ...
                (fold_dir/"relax_note.json").write_text(json.dumps({
        ...
        (fold_dir/"label_counts.json").write_text(json.dumps(counts, indent=2), encoding="utf-8")
        step.update(1)
        ...
        for side in ["long","short"]:
            ...
            with open(gdir / "meta.json", "w", encoding="utf-8") as f:
                json.dump({"side": side, "symbol": symbol, "train_months": train_m}, f, indent=2)
            ...
            for k in range(K):
                ...
                for side in ["long","short"]:
                    ...
                    with open(reg_dir / "meta.json", "w", encoding="utf-8") as f:
                        json.dump({"side": side, "symbol": symbol, "train_months": train_m, "regime": k}, f, indent=2)
        if per_regime_banks_meta:
            with open(art_dir/"regime_banks_meta.json","w",encoding="utf-8") as f:
                json.dump(per_regime_banks_meta, f, indent=2)
        step.update(1)
